...
/*   Updated: 2024/04/24 14:28:58 by mhotting         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	bi_exit_shell(t_minishell *shell, int status, char *msg)
{
	if (status == STATUS_EXIT_BAD_ARG)
		...
		ft_dprintf(STDERR_FILENO, EXIT_MSG_LITERAL, msg);
	if (shell != NULL)
...
int	bi_exit(t_minishell *shell, char **argv)
{
	...
	if (argv[2] != NULL)
	{
		ft_dprintf(STDERR_FILENO, EXIT_MSG_LITERAL, EXIT_MSG_TOO_MANY_ARGS);
		return (EXIT_FAILURE);
	}
